...
class Gestures {
    ...
    static async checkIfDisplayedWithSwipeDown(
        ...
        if (!(await element.isDisplayed()) && amount <= maxScrolls) {
            ...
            await this.checkIfDisplayedWithSwipeDown(
                element,
                maxScrolls,
                amount + 1
            );
    ...
    static async checkIfDisplayedWithSwipeDownDrop(
        element: WebdriverIO.Element,
        maxScrolls: number,
        amount = 0
    ) {
        ...
        if (!(await element.isDisplayed()) && amount <= maxScrolls) {
            ...
            await this.checkIfDisplayedWithSwipeDownDrop(
                element,
                maxScrolls,
                amount + 1
            );
    ...
    static async checkIfDisplayedWithSwipeLeft(
        ...
        if (!(await element.isDisplayed()) && amount <= maxScrolls) {
            ...
            await this.checkIfDisplayedWithSwipeLeft(
                element,
                maxScrolls,
                amount + 1,
                yPrecentage,
        ...
        } else if (amount > maxScrolls && !expectedError) {
            // If the element is still not visible after the max amount of scroll let it fail
            throw new Error(
                `The element '${element}' could not be found or is not visible.`
            );
    ...
    static async checkIfDisplayedWithSwipeLeftFilter(
        element: WebdriverIO.Element,
        maxScrolls: number,
        amount = 0,
        yPrecentage: number = 50,
    ...
    static async customCheckElementSwipeUp(
        element: WebdriverIO.Element,
        maxScrolls: number,
        xPrecentage: number = 99,
        amount = 0
    ) {
        // If the element is not displayed and we haven't scrolled the max amount of scrolls
        // then scroll and execute the method again
        if (!(await element.isDisplayed()) && amount <= maxScrolls) {
            ...
            await this.customCheckElementSwipeUp(
                element,
                maxScrolls,
                xPrecentage,
                amount + 1
    ...
    static async customSwipePopupOpsiPembayaran(
        from: number,
        to: number,
        element: WebdriverIO.Element,
        maxScrolls: number,
    ...
    static async customCheckElementSwipe(
        element: WebdriverIO.Element,
        maxScrolls: number,
        amount = 0,
        i: number,
        ...
        // then scroll and execute the method again
        if (!(await element.isDisplayed()) && amount <= maxScrolls) {
            await Gestures.swipeOnPercentage({ x: 40, y: i }, { x: 40, y: j });
            await this.customCheckElementSwipe(
                element,
                maxScrolls,
                amount + 1,
                i,
    ...
    //costumize @lcsmoehr0020
    static async customTapPercentage(from: XY) {
        // Get the screen size and store it so it can be re-used.
        // This will save a lot of webdriver calls if this methods is used multiple times.
        SCREEN_SIZE = SCREEN_SIZE || (await driver.getWindowRect());
        // Get the start position on the screen for the swipe
        const pressOptions = this.getDeviceScreenCoordinates(SCREEN_SIZE, from);
        // Get the move to position on the screen for the swipe
        await this.customTap(pressOptions);
    }

    static async customTap(from: XY) {
        await driver.performActions([
            {
                // a. Create the event
                type: 'pointer',
    ...
    static async CustomSwipePercent(from: XY, to: XY) {
        // Get the screen size and store it so it can be re-used.
        // This will save a lot of webdriver calls if this methods is used multiple times.
        SCREEN_SIZE = SCREEN_SIZE || (await driver.getWindowRect());
        // Get the start position on the screen for the swipe
